...
export async function POST(request: Request) {
  ...
  const data = await regenerateCreatorTrendAiRecommendations({ direction, platform, keyword });

  return timedJson(
    { data },
    {
      metrics: [{ name: "app", durMs: timer.elapsedMs(), desc: "creator-trend-ai-recommendations-regenerate" }],
    },
  );
}
