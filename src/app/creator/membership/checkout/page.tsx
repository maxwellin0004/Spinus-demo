        ...
        <div className="grid gap-4 px-6 py-6 lg:grid-cols-[0.92fr_1.08fr]">
          ...
          <div className="grid gap-4">
            {creatorMembershipTiers.map((tier) => (
              <div className="rounded-[24px] border border-stone-200 bg-white p-5" key={tier.slug}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-black uppercase tracking-[0.16em] text-stone-500">{tier.badge}</p>
                    <h3 className="mt-2 text-2xl font-black text-stone-950">{tier.title}</h3>
                  </div>
                  <span className="rounded-full bg-stone-950 px-4 py-2 text-sm font-black text-white">{tier.priceLabel}</span>
                </div>
                <p className="mt-4 text-sm leading-7 text-stone-600">{tier.subtitle}</p>
