...
function StatCard({
  ...
  return (
    <div className="flex min-h-24 items-center gap-4 border-b border-stone-200/80 bg-white/76 px-5 py-4 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0">
...
function ViewToggle({ value, onChange }: { value: ViewMode; onChange: (value: ViewMode) => void }) {
  return (
    <div className="inline-flex rounded-lg border border-stone-200 bg-white p-1 shadow-sm" aria-label="任务视图切换">
      <button
...
function TaskCard({ task }: { task: MarketplaceTask }) {
  return (
    <article className="group flex min-h-[19rem] flex-col rounded-lg border border-stone-200 bg-white/88 p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-amber-200 hover:shadow-md">
      ...
      <div className="mt-4">
        ...
        <p className="mt-1 text-sm text-stone-500">{task.brandName} · {task.campaignTitle}</p>
      </div>
...
function TaskTable({ tasks }: { tasks: MarketplaceTask[] }) {
  if (tasks.length === 0) return <EmptyTasks />;
  return (
    <div className="overflow-hidden rounded-lg border border-stone-200 bg-white/88 shadow-sm">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1080px] text-left text-sm">
          <thead className="border-b border-stone-200 bg-stone-50 text-xs text-stone-500">
            <tr>
              <th className="px-4 py-3 font-black">任务</th>
              <th className="px-4 py-3 font-black">平台</th>
              <th className="px-4 py-3 font-black">奖励</th>
              <th className="px-4 py-3 font-black">剩余额度</th>
              <th className="px-4 py-3 font-black">粉丝要求</th>
              <th className="px-4 py-3 font-black">截止时间</th>
              <th className="px-4 py-3 font-black">适配度</th>
              ...
              <tr className={cn("align-middle transition hover:bg-amber-50/40", index === 0 && task.isRecommended ? "bg-amber-50/55" : "")} key={task.id}>
                <td className="px-4 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-black text-stone-950">{task.title}</p>
                      <p className="mt-1 text-xs text-stone-500">{task.brandName} · {task.industry}</p>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-4">
                  <span className={cn("inline-flex rounded-full border px-3 py-1 text-xs font-black", platformTone(task.platform))}>{task.platform}</span>
                </td>
                <td className="px-4 py-4">
          ...
          <>
            <div className="mt-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-stone-500">{featured.brandName} · {currency(featured.rewardAmount)}</p>
  ...
  return (
    <div className="grid gap-5">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-stone-950">任务大厅</h1>
          <p className="mt-2 text-sm text-stone-500">海量品牌合作任务，找到适合你账号的优质机会</p>
        </div>
        <ViewToggle value={viewMode} onChange={setViewMode} />
      </section>

      <section className="grid overflow-hidden rounded-lg border border-stone-200 bg-white/72 shadow-sm md:grid-cols-2 xl:grid-cols-4">
      ...
      <form className="grid gap-3 rounded-lg border border-stone-200 bg-white/80 p-4 shadow-sm xl:grid-cols-[1.6fr_1fr_1fr_1fr_1fr_auto_auto]" action="/creator/marketplace">
        ...
        <select className="h-11 rounded-lg border border-stone-200 bg-white px-3 text-sm outline-none focus:border-amber-300 focus:ring-4 focus:ring-amber-100" name="platform" defaultValue={currentFilters.platform ?? ""}>
          <option value="">全部平台</option>
          {filterOptions.platforms.map((item) => <option key={item} value={item}>{item}</option>)}
        </select>
        <select className="h-11 rounded-lg border border-stone-200 bg-white px-3 text-sm outline-none focus:border-amber-300 focus:ring-4 focus:ring-amber-100" name="country" defaultValue={currentFilters.country ?? ""}>
          <option value="">全部地区</option>
          {filterOptions.countries.map((item) => <option key={item} value={item}>{item}</option>)}
        </select>
        <select className="h-11 rounded-lg border border-stone-200 bg-white px-3 text-sm outline-none focus:border-amber-300 focus:ring-4 focus:ring-amber-100" name="industry" defaultValue={currentFilters.industry ?? ""}>
          <option value="">全部分类</option>
          {filterOptions.industries.map((item) => <option key={item} value={item}>{item}</option>)}
        </select>
        <select className="h-11 rounded-lg border border-stone-200 bg-white px-3 text-sm outline-none focus:border-amber-300 focus:ring-4 focus:ring-amber-100" name="minReward" defaultValue={currentFilters.minReward ?? ""}>
        ...
        <button className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-stone-950 px-5 text-sm font-black text-white hover:bg-stone-800" type="submit">
          <Filter className="h-4 w-4" />
          筛选
        </button>
        <Link className="inline-flex h-11 items-center justify-center rounded-lg px-4 text-sm font-black text-stone-500 hover:bg-stone-50 hover:text-stone-950" href="/creator/marketplace">
          重置
        </Link>
      </form>

      <section className="flex flex-col gap-3 rounded-lg border border-stone-200 bg-white/70 px-4 py-3 shadow-sm md:flex-row md:items-center md:justify-between">
        <div className="flex flex-wrap gap-5">
          {tabs.map((tab) => {
            ...
            return (
              <Link
                className={cn(
                  "relative inline-flex h-9 items-center gap-2 text-sm font-black transition",
